...
var century_heatmapLayer = new HeatmapOverlay(century_cfg)
var century_group = L.layerGroup()

function century_fetchData() {
  fetch("JSONS/Albuquerque_CenturyLink.json")
      .then((res) => {
      return res.json();
  })
  .then((data) => {
    century_heatmap = data;
    console.log(century_heatmap);
    // Use the heatmap data
    century_heatmapLayer.addData(century_heatmap);
    century_group.addLayer(century_heatmapLayer)
    // Add popups for each heatmap point
    data.forEach(point => addPopup(point, century_group));
  });
}
...
var map = new L.Map('map', {
  center: new L.LatLng(35.0844, -106.6504), // HERE is where the cities Lat and Long go to center the map
  minZoom: 9,
  maxZoom: 15,
  zoom: 10,
  layers: [baseLayer, century_group]
})

var overlayMaps = {
  "Century Link": century_group
};
